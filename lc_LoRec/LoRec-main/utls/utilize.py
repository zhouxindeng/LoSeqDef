...
def custom_loss(scores, labels, alpha=0.05, beta=1.0):
    labels = labels.unsqueeze(-1).float().to(scores.device) #torch.Size([567, 1])
    
    pos_weight = torch.tensor(beta).to(scores.device)   #正样本的权重
    neg_weight = torch.tensor(1.).to(scores.device)     #负样本的权重
    
    weights = torch.where(labels == 1, pos_weight, neg_weight) #根据标签的值（1 或 0）为每个样本分配权重torch.Size([567, 1])
    
    bce_loss = F.binary_cross_entropy(scores, labels, weight=weights, reduction='none') #LF
    entropy = -scores * torch.log(scores + 1e-10) - (1 - scores) * torch.log(1 - scores + 1e-10)
    entropy_regularization = torch.where(labels == 0, entropy, torch.zeros_like(entropy))#熵正则化LER，对于用户集的用户（labels == 0），应用熵正则化；对于攻击注入用户（labels == 1），正则化项为零。
    total_loss = bce_loss + alpha * entropy_regularization
    
    return total_loss.mean()#LLLM4DEc = LF + 𝜆LER,

def slice_lists(list1, list2, batch_size):
    len1, len2 = len(list1), len(list2)
    num_batches = -(-max(len1, len2) // batch_size) #12
    
    slice_size1 = -(-len1 // num_batches)
    slice_size2 = -(-len2 // num_batches)
    
    slices1 = [list1[i:i + slice_size1] for i in range(0, len1, slice_size1)]  #[num_batches:12,slice_size1:509]
    slices2 = [list2[i:i + slice_size2] for i in range(0, len2, slice_size2)]  #[num_batches:12,slice_size2:58]
