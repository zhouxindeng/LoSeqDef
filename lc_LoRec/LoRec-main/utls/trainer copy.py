...
class SASrecTrainer(BasicTrainer):
    def __init__(self, trainer_config) -> None:
        ...
        self._create_dataset(f"F:/GraduationProject/GraduationProject/lc_LoRec/LoRec-main/data/{trainer_config['dataset']}")
    ...
    def _train_lct_epoch(self, epoch):
        ...
        for users, fakes in zip(user_list, fake_list):
            ...
            lct_predict, similarity = self.lct_model(seq=model_predict_emb, detections=detections, mask=mask) #lct_predict.size()=torch.Size([567, 1]),similarity.size()=torch.Size([])
            lct_loss = self._get_lct_loss(lct_predict, label, similarity)  #LLCT= LF + 𝜆1LER + 𝜆2LLLM,
            lct_loss.backward()
            self.opt_lct.step()
    ...
    def _get_lct_loss(self, lct_predict, label, similarity):
        return custom_loss(lct_predict, label, alpha=self.config["reg_entropy"]) - self.config["sim_weight"] * similarity #= LF + 𝜆1LER + 𝜆2LLLM,
