...
class Llama2(BasicLLM):
    ...
    def _create_LLM(self):
        token = '-----------'
        model = "G:/GraduationProject/lc_LoRec/Llama-2-7b-chat-hf"
        self.pipeline = transformers.pipeline(
            task="feature-extraction",
            model=model,
            torch_dtype=torch.float16,
        ...
        self.pipeline2 = transformers.pipeline(
    ...
    def text_LLM(self, input_text):
        with torch.no_grad():
            result = self.pipeline2(input_text, do_sample=False)
