...
class BasicTrainer:
    ...
    def _create_dataloader(self):
        self.dataloader = DataLoader(self.dataset, batch_size=self.config["batch_size"], shuffle=True)  #bath_size=512
    ...
    def _init_path(self, path=None):
        if self.config["use_LLM"]: 
            best_model_path = f"/Users/changliu/Documents/GraduationProject/lc_LoRec/{self.config['checkpoints']}/{self.config['model']}_{self.config['LLM']}/{self.config['dataset']}"
            if self.config["with_lct"]:
                best_model_path = f"/Users/changliu/Documents/GraduationProject/lc_LoRec/{self.config['checkpoints']}/{self.config['model']}_{self.config['LLM']}/{self.config['dataset']}/FD"
        else:
            best_model_path = f"/Users/changliu/Documents/GraduationProject/lc_LoRec/{self.config['checkpoints']}/{self.config['model']}_NoLLM/{self.config['dataset']}"
    ...
    def train(self, path=None):
        ...
        #best_model_path="G:/GraduationProject/lc_LoRec/checkpoints/SASrec_Llama2_13/Test/FD/20250224/attack_random_1_202502240857.pth"
        
        self.update_flag = False
        #for epoch in range(self.n_epochs):
        for epoch in range(1):
            self._train_epoch(epoch)
            self._update_lct(epoch)     #更新
...
class SASrecTrainer(BasicTrainer):
    def __init__(self, trainer_config) -> None:
        ...
        self._create_dataset(f"/Users/changliu/Documents/GraduationProject/lc_LoRec/LoRec-main/data/{trainer_config['dataset']}")
    ...
    def _train_epoch(self, epoch):
        ...
        for batch_data in self.dataloader:
            self.opt.zero_grad()
            self.model.train()
            seq, mask, neg = self.dataset.get_train_batch(batch_data) #seq.size()=neg.size()=torch.Size([batch_size:512, max_len+1:51, LLM_size:5120]);mask_list=[512,51]
            predict_emb, target_pos_embs, target_neg_embs, _ = self.model(seq, mask, neg) #predict_emb.size()=torch.Size([bs:512, max_len:50, MLP_layer:512]),target_pos_embs.size()=torch.Size([512, 50, 512]),target_neg_embs.size()=torch.Size([512, 50, 512])
            pos_logits = (predict_emb * target_pos_embs).sum(-1)    #逐元素相乘，并沿最后一个维度求和，pos_logits.size()=torch.Size([512, 50])
        ...
        if self.config["with_lct"]:
            for _ in range(self.config["lct_minibatch"]):#2
    ...
    def _train_lct_epoch(self, epoch):
        ...
        user_list, fake_list = slice_lists(all_user, all_fake, self.config["batch_size"]) #user_list [num_batches:12,n_users/num_batches:509] fake_list #[num_batches:12,n_fake_users/num_batches:58]
        for users, fakes in zip(user_list, fake_list):
            self.opt_lct.zero_grad()
            emb, mask, label, detections = self.dataset.get_fake_user_batch(neg_idx=users, fk_idx=fakes)  #emb.size()=torch.Size([567, 51, 5120]),mask.size()=[567,51],label.size()=torch.Size([567]),detections.size()=torch.Size([567, 5120])
            with torch.no_grad():
                model_predict_emb = self.model.get_emb(emb, mask) #model_predict_emb.size()=#torch.Size([567, 51, 1024])
            lct_predict = self.lct_model(seq=model_predict_emb, detections=detections, mask=mask) #lct_predict.size()=torch.Size([567, 1]),similarity.size()=torch.Size([])
            lct_loss = self._get_lct_loss(lct_predict, label)  #LLCT= LF + 𝜆1LER + 𝜆2LLLM,
            lct_loss.backward()
            self.opt_lct.step()
    ...
    def test_model(self, model_path=None):
        ...
        cnt = 0
        for batch_data in self.dataloader:
            org_seq, seq, mask, pos = self.dataset.get_test_batch(batch_data)#len(idx)真实用户数量[len(idx),51],[len(idx),51,5120],[len(idx),50],[len(idx)]
            if len(pos) == 0:
                continue
            cnt += len(pos)
            hr, ndcg = self._eval_rec(org_seq, seq, mask, pos)
    ...
    def _eval_rec(self, org_seq, seq, mask, pos):
        ...
        if self.config["use_LLM"]:
            all_idx = self.dataset.get_labels_emb(all_idx) #[n_items,5120]
        with torch.no_grad():
            all_logits = self.model.predict(seq, mask, all_idx)#[len(idx),n_items]
    ...
    def _update_weight(self):
        ...
        for users, fakes in zip(user_list, fake_list):
            self.model.eval()
            self.lct_model.eval()
            emb, mask, label, _ = self.dataset.get_fake_user_batch(neg_idx=users, final_test=0, fk_idx=fakes)#emb.size()=torch.Size([512, 51, 5120]),mask.size()=[512,51],label.size()=torch.Size([512])
            with torch.no_grad():
                model_predict_emb = self.model.get_emb(emb, mask) #torch.Size([512, 51, 1024])
            ...
            for user, user_score in zip(users, score_list):
                user_score_dict[user] = user_score #userID:user是欺诈者的得分
    ...
    def _get_rec_loss(self, pos_logits, neg_logits, mask, lct_weight=None):#pos_logits:[512,50],neg_logits:[512,50],mask:[512,51]
        pos_labels, neg_labels = torch.ones(pos_logits.shape, device=self.device), torch.zeros(neg_logits.shape, device=self.device)
        indices = torch.where(torch.FloatTensor(mask)[:,:-1] != 0) #torch.where 获取非0元素的行，列索引 indices.size()=[2,11139]
        if lct_weight is None:
            ...
            expanded_lct_weight = lct_weight.expand_as(pos_logits)[indices]#torch.Size([11139])
    ...
    def _get_lct_loss(self, lct_predict, label):
        return custom_loss(lct_predict, label, alpha=self.config["reg_entropy"]) #= LF + 𝜆1LER + 𝜆2LLLM,
