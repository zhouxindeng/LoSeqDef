...
class LLMSASrec(torch.nn.Module):
    ...
    def forward(self, interaction_list, interaction_mask, neg_list):
        ...
        # positive_mask=torch.eye(cos_sim.size(0)).bool().to(cos_sim.device)
        # negative_mask=~positive_mask
        # positive_scores = torch.sum(torch.log(torch.exp(cos_sim) / torch.sum(torch.exp(cos_sim), dim=1, keepdim=True)) * positive_mask, dim=1)
        # negative_scores = torch.sum(torch.log(torch.exp(cos_sim) / torch.sum(torch.exp(cos_sim), dim=1, keepdim=True)) * negative_mask, dim=1)
        # cmcl_loss = -torch.mean(positive_scores - torch.logsumexp(negative_scores, dim=0))
    ...
    def predict(self, interaction_list, interaction_mask, item_indices):#interaction_list:torch.Size([512(len(idx)非欺诈者用户), 51, 5120]),interaction_mask:[512,51],item_indices:torch.Size([28638,5120])
        log_mask = torch.FloatTensor(interaction_mask).to(self.config["device"])
        ...
        logits = item_embs.matmul(prec_vec.unsqueeze(-1)).squeeze(-1) #torch.Size([512, 28638])，[28638,512]->[512,28639,512]*[512,512,1]->[512,28639,1]->[512,28639]
    ...
    def get_emb(self, interaction_list, interaction_mask):  #emb.size()=torch.Size([567, 51, 5120]),mask.size()=[567,51]
        ...
        return torch.cat((input_embs_all, prec_vec), dim=2) #torch.Size([567, 51, 1024])
