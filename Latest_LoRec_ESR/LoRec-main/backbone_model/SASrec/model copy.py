...
from backbone_model.SASrec.CrossAttention import Multi_CrossAttention
...
class LLMSASrec(torch.nn.Module):
    def __init__(self, config):
        ...
        self.cross_att = Multi_CrossAttention(hidden_size=config["hidden_units"],all_head_size=config["hidden_units"], head_num=config["num_heads"])

    def forward(self, interaction_list, interaction_mask, neg_list, pos_interaction, neg_interaction):
        ...
        pos_interaction = torch.tensor(pos_interaction, dtype=torch.long, device=self.config["device"])
        neg_interaction = torch.tensor(neg_interaction, dtype=torch.long, device=self.config["device"])
        input_embs_all = self.fc(interaction_list)   # LLM特征投影 [bathsize, max_len+1, LLM_size] -> [bathsize, max_len+1, layers(512)]
        input_embs_all *= (self.config["hidden_units"] ** 0.5)  # 尺度缩放
        input_embs_all = self.emb_dropout(input_embs_all)  # dropout

        col_embs_pos=self.col_emb(pos_interaction)   #torch.Size([512, 51, 512])
        col_embs_pos *= (self.config["hidden_units"] ** 0.5)  # 尺度缩放
        col_embs_pos = self.emb_dropout(col_embs_pos)  # dropout
        #input_logs_embs = self.cross_att(col_embs_pos, input_embs_all, log_mask)
        input_logs_embs = 0.1*col_embs_pos+input_embs_all

        #input_logs_embs = input_embs_all
        target_pos_embs = input_logs_embs[:, 1:, :]
        
        neg_list=self.fc(neg_list)
        neg_list *= (self.config["hidden_units"] ** 0.5)  # 尺度缩放
        neg_list = self.emb_dropout(neg_list)  # dropout

        col_embs_neg=self.col_emb(neg_interaction)
        col_embs_neg *= (self.config["hidden_units"] ** 0.5)  # 尺度缩放
        col_embs_neg = self.emb_dropout(col_embs_neg)  # dropout
        #neg_list=self.cross_att(col_embs_neg,neg_list, log_mask)
        neg_list=0.1*col_embs_neg+neg_list
        target_neg_embs = neg_list[:, :-1, :]  #[bathsize, max_len, LLM_size]
        prec_vec = self.user_encoder(input_logs_embs, log_mask, self.config["device"])    # transformer序列编码结果（含LLM特征）torch.Size([512, 51, 512])

        return prec_vec[:, :-1, :], target_pos_embs, target_neg_embs, torch.cat((input_embs_all, prec_vec), dim=2)  # 特征拼接：LLM原始特征 + 序列编码结果 [512, 51, 2*512]
    
    def predict(self, interaction_list, interaction_mask, item_indices, pos_interaction):#interaction_list:torch.Size([512, 51, 5120]),interaction_mask:[512,51],item_indices:torch.Size([28638,5120])
        log_mask = torch.FloatTensor(interaction_mask).to(self.config["device"])
        pos_interaction = torch.tensor(pos_interaction, dtype=torch.long, device=self.config["device"])

        input_logs_embs = self.fc(interaction_list)#torch.Size([512, 51, 512])
        col_embs_pos=self.col_emb(pos_interaction)
        #input_logs_embs = self.cross_att(col_embs_pos, input_logs_embs, log_mask)
        input_logs_embs = 0.1*col_embs_pos+input_logs_embs
    ...
    def get_emb(self, interaction_list, interaction_mask, pos_interaction):  #emb.size()=torch.Size([567, 51, 5120]),mask.size()=[567,51]
        log_mask = torch.FloatTensor(interaction_mask).to(self.config["device"])
        pos_interaction = torch.tensor(pos_interaction, dtype=torch.long, device=self.config["device"])

        input_embs_all = self.fc(interaction_list)  #torch.Size([567, 51, 512])
        col_embs_pos=self.col_emb(pos_interaction)
        #input_embs_all= self.cross_att(col_embs_pos, input_embs_all, log_mask)
        input_embs_all = 0.1*col_embs_pos+input_embs_all
        input_logs_embs=input_embs_all

        prec_vec = self.user_encoder(input_logs_embs, log_mask, self.config["device"])

        pos_logits = (prec_vec * input_logs_embs).sum(-1) #逐元素相乘，并沿最后一个维度求和，pos_logits.size()=torch.Size([567, 51])
        logits = (1-torch.sigmoid(pos_logits)).unsqueeze(2)

        return torch.cat((input_embs_all, logits, prec_vec, logits), dim=2) #torch.Size([567, 51, 1024]), logits #torch.Size([567])
