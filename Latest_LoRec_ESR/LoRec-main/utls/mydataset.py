...
class BasicDataset(Dataset):
    ...
    def get_fake_user_batch(self, size=512, neg_idx=None, final_test=0, fk_idx=None, inj_idx=None): #len(neg_idx)=509,len(fk_idx)=58
        if final_test != 0:
            ...
            interaction_list_pos = interaction_list
        else:
        ...
        return interaction_list, mask_list, torch.tensor(fd_label), torch.stack(detection_list).float().to(self.config["device"]), interaction_list_pos
...
class SASDataset(BasicDataset):
    ...
    def get_train_batch(self, idx):
        ...
        for id in idx:
            ...
            neg_interaction_list.append(random.sample(whole_item_set - interaction_set, len(interaction_list[:-2])))    #随机不重复采样
        ...
        interaction_list_pos = all_interaction_list
        interaction_list_neg = neg_interaction_list
        ...
        return all_interaction_list, mask_list, neg_interaction_list, interaction_list_pos, interaction_list_neg #shape=torch.Size([batch_size:512, max_len+1:51, LLM_size:5120]);mask_list.shape=[512,51]
