...
class BasicTrainer:
    ...
    def _create_dataloader(self):
        self.dataloader = DataLoader(self.dataset, batch_size=self.config["batch_size"], shuffle=True,num_workers=14)  #bath_size=512
...
class SASrecTrainer(BasicTrainer):
    def __init__(self, trainer_config) -> None:
        ...
        self._create_dataset(f"/root/tf-logs/Latest_LoRec/LoRec-main/data/{trainer_config['dataset']}")
    ...
    def _train_epoch(self, epoch):
        ...
        for batch_data in self.dataloader:
            ...
            predict_emb, target_pos_embs, target_neg_embs, _ , similarity= self.model(seq, mask, neg) #predict_emb.size()=torch.Size([bs:512, max_len:50, MLP_layer:512]),target_pos_embs.size()=torch.Size([512, 50, 512]),target_neg_embs.size()=torch.Size([512, 50, 512])
            ...
            if self.config["with_lct"] and (epoch + 1) >= self.config["lct_start"]:
                ...
                loss = self. _get_rec_loss(pos_logits, neg_logits, mask, similarity, weight)
            else:
                loss = self._get_rec_loss(pos_logits, neg_logits, mask, similarity)
            loss.backward()
            self.opt.step()
    ...
    def _train_lct_epoch(self, epoch):
        ...
        for users, fakes in zip(user_list, fake_list):
            ...
            lct_loss = self._get_lct_loss(lct_predict, label, similarity)  #LLLM4DEc = LF + 𝜆LER,
            lct_loss.backward()
            self.opt_lct.step()
    ...
    def _get_rec_loss(self, pos_logits, neg_logits, mask, similarity, lct_weight=None):#pos_logits:[512,50],neg_logits:[512,50],mask:[512,51]
        ...
        return weighted_pos_loss + weighted_neg_loss - self.config["sim_weight"] * similarity
