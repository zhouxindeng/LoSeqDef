...
class TransformerEncoder(torch.nn.Module):
    def __init__(self, n_vocab, n_position, d_model, n_heads, dropout, n_layers):
        ...
        self.position_embedding = nn.Embedding(n_position, d_model) #[(max_len):51,(hidden_units):512]
        self.dropout = nn.Dropout(p=dropout)
        self.layer_norm = nn.LayerNorm(d_model, eps=1e-6)
    ...
    def forward(self, input_embs, log_mask, att_mask):  # input_embs.size()=torch.Size([(bs)512, (max_len+1)51, (layer)512]);log_mask.size()=torch.Size([512, 51]);att_mask.size()=torch.Size([512, 1, 51, 51])
        position_ids = torch.arange(log_mask.size(1), dtype=torch.long, device=log_mask.device)
        position_ids = position_ids.unsqueeze(0).expand_as(log_mask)     #torch.Size([512, 51])
        output = self.layer_norm(input_embs + self.position_embedding(position_ids))   #torch.Size([512, 51, 512])
