...
class LLMGRU4rec(nn.Module):
    ...
    def get_emb(self, interaction_list, interaction_mask):
        ...
        pos_logits = (final_output[:,:-1,:] * input_embs_all[:,1:,]).sum(-1) #逐元素相乘，并沿最后一个维度求和，pos_logits.size()=torch.Size([567, 51])
        ...
        return torch.cat((input_embs_all, logits, final_output, logits), dim=2)
